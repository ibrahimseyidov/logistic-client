...
import panelStyles from "../../musteriler/musteriler.module.css";
import drawerStyles from "./CarrierCreateDrawer.module.css";
...
export interface CreatedCarrierSummary {
  id: string;
  name: string;
  company?: string;
  shortName?: string;
...
interface CarrierCreateDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  onCreated?: (carrier: CreatedCarrierSummary) => void;
}

export function CarrierCreateDrawer({
  isOpen,
  onClose,
  onCreated,
}: CarrierCreateDrawerProps) {
  ...
  const handleCreateContactPerson = async (data: ContactPersonFormData) => {
    try {
      ...
      dispatch(showNotification({ message: "Yeni daşıyıcı əlaqədar şəxs əlavə edildi", type: "added" }));
    } catch {
  ...
  const handleCreateCarrier = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        carrierType: getLookupLabel(form.carrierType, carrierTypeOptions) || "Yeni daşıyıcı",
      ...
      const created = await createCarrierAction(payload);
      pendingDocumentFilesRef.current.clear();
      dispatch(showNotification({ message: "Daşıyıcı uğurla yaradıldı", type: "success" }));
      onCreated?.({
        id: String(created?.id ?? ""),
        name: created?.name || form.company.trim(),
        company: created?.company || form.company.trim(),
        shortName: created?.shortName || form.shortName.trim(),
        carrierType: created?.carrierType || payload.carrierType,
      });
      handleClose();
    } catch {
      dispatch(showNotification({ message: "Daşıyıcı yaradılarkən xəta baş verdi", type: "error" }));
    }
  };
      ...
      <aside className={`${layoutStyles.drawer} ${layoutStyles.drawerOpen}`}>
        <div className={panelStyles.newPanel}>
          <div className={panelStyles.newPanelHeader}>
            <div>
              <h2 className={panelStyles.newPanelTitle}>Yeni daşıyıcı</h2>
                  ...
                  <div className={panelStyles.inlineControlRow}>
                    ...
                    >
                      <Select
                        ...
                        placeholder="-"
                        onChange={(value) => setForm((prev) => ({ ...prev, carrierType: value }))}
                        onOpenChange={(open) => {
          ...
          <div className={panelStyles.newPanelFooter}>
            ...
            <button type="button" className={panelStyles.applyButton} onClick={handleCreateCarrier}>
              Yaddaşda saxlamaq
            </button>
          </div>
        </div>
